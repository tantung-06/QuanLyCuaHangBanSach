...
public class NhanVienPanel extends JPanel {
    ...
    private JPanel buildTablePanel() {
        ...
        String[] cols = {"Mã NV", "Họ", "Tên", "Số điện thoại",
                         "Địa chỉ", "Email", "Mã tài khoản", "Trạng thái"};
    ...
    public void loadData() {
        ...
        if ("Tất cả".equals(filter))
            danhSach = nvBUS.getAll();
        else
            danhSach = nvBUS.getByTrangThai(filter);
        renderTable(danhSach);
    }

    private void renderTable(ArrayList<NhanVien> ds) {
        tableModel.setRowCount(0);
        for (NhanVien n : ds) {
            tableModel.addRow(new Object[]{
                n.getMaNhanVien(), n.getHo(), n.getTen(),
                n.getSoDienThoai(), n.getDiaChi(), n.getEmail(),
                n.getMaTaiKhoan(), n.getTrangThai()
            });
        }
    }
    ...
    private void handleSearch() {
        ...
        ArrayList<NhanVien> filtered = new ArrayList<>();
        for (NhanVien n : danhSach) {
            if (n.getMaNhanVien().toLowerCase().contains(kw)
                || n.getHo().toLowerCase().contains(kw)
                || n.getTen().toLowerCase().contains(kw)
                || (n.getSoDienThoai() != null && n.getSoDienThoai().contains(kw))
    ...
    private NhanVien getSelected() {
        ...
        return nvBUS.getById((String) tableModel.getValueAt(row, 0));
    }

    private void handleThem() {
        NhanVienDialog d = new NhanVienDialog(getFrame(), null);
        d.setVisible(true);
        if (d.isSaved()) loadData();
    }

    private void handleSua() {
        NhanVien n = getSelected();
        if (n == null) return;
        NhanVienDialog d = new NhanVienDialog(getFrame(), n);
        d.setVisible(true);
        if (d.isSaved()) loadData();
    }
    ...
    private void handleXoa() {
        ...
        int confirm = JOptionPane.showConfirmDialog(this,
                "Xác nhận xóa nhân viên: \"" + n.getHo() + " " + n.getTen() + "\"?",
                "Xác nhận xóa", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
        if (confirm != JOptionPane.YES_OPTION) return;
        if (nvBUS.delete(n.getMaNhanVien())) {
            JOptionPane.showMessageDialog(this, "Xóa thành công!");
            loadData();
        } else {
            JOptionPane.showMessageDialog(this,
                    "Xóa thất bại! Nhân viên có thể đang có phiếu nhập/xuất.",
