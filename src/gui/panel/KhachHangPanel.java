...
public class KhachHangPanel extends JPanel {
    ...
    private JPanel buildTablePanel() {
        ...
        String[] cols = {"Mã KH", "Họ", "Tên", "Số điện thoại",
    ...
    public void loadData() {
        ...
        if ("Tất cả".equals(filter))
            ...
            danhSach = khBUS.getByTrangThai(filter);
        renderTable(danhSach);
    }

    private void renderTable(ArrayList<KhachHang> ds) {
        tableModel.setRowCount(0);
        for (KhachHang k : ds) {
            tableModel.addRow(new Object[]{
                k.getMaKhachHang(), k.getHo(), k.getTen(),
                k.getSoDienThoai(), k.getDiaChi(), k.getEmail(),
                k.getTrangThai()
            });
        }
    }
    ...
    private void handleSearch() {
        ...
        for (KhachHang k : danhSach) {
            if (k.getHo().toLowerCase().contains(kw)
                || k.getTen().toLowerCase().contains(kw)
                || k.getMaKhachHang().toLowerCase().contains(kw)
                || (k.getSoDienThoai() != null && k.getSoDienThoai().contains(kw))
    ...
    private KhachHang getSelected() {
        ...
        return khBUS.getById((String) tableModel.getValueAt(row, 0));
    }

    private void handleThem() {
        KhachHangDialog d = new KhachHangDialog(getFrame(), null);
        d.setVisible(true);
        if (d.isSaved()) loadData();
    }

    private void handleSua() {
        KhachHang k = getSelected();
        if (k == null) return;
        KhachHangDialog d = new KhachHangDialog(getFrame(), k);
        d.setVisible(true);
        if (d.isSaved()) loadData();
    }
    ...
    private void handleXoa() {
        ...
        int confirm = JOptionPane.showConfirmDialog(this,
                "Xác nhận xóa khách hàng: \"" + k.getHo() + " " + k.getTen() + "\"?",
                "Xác nhận xóa", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
        if (confirm != JOptionPane.YES_OPTION) return;
        if (khBUS.delete(k.getMaKhachHang())) {
            JOptionPane.showMessageDialog(this, "Xóa thành công!");
            loadData();
        } else {
            JOptionPane.showMessageDialog(this,
                    "Xóa thất bại! Khách hàng có thể đang có phiếu xuất.",
